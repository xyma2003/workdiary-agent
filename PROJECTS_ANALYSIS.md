...
- 审阅页增加保存前事实检查，提示来源中未出现的数字和缺少量化指标声明
- 历史页支持正文搜索、模板/日期筛选和分页，放弃失败任务时清理未入库导出文件
- 事实检查以原始输入/Git/粘贴数据为可信源，不再让模型生成的指标摘要自证正确；潜在凭证阻止保存
- 跨天恢复按日报日期读取 Git，并对作者身份做二次严格匹配；保存失败自动回滚新导出
- 完成任务后清理 checkpoint，离线图测试使用临时存储，不再污染真实历史和导出目录

**仍待完成：**
- TemplateRouterAgent 已有 30 条平衡标注集和评测脚本，但尚未运行真实供应商基线与人工复核
- 规则型事实风险提示已具备，但仍缺少语义级事实一致性 Benchmark 和成本/延迟统计
